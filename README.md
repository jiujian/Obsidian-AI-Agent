...
- **智谱 AI (GLM)**: 支持 glm-4 和 glm-4-flash 模型，支持 MCP（模型上下文协议）
- **OpenAI**: 支持 GPT-4o 和 GPT-4o-mini 模型
- **Kimi (Moonshot)**: 支持 moonshot-v1-8k 和 moonshot-v1-32k 模型
...
**智谱 AI MCP（模型上下文协议）配置**：

插件支持智谱 AI 的 MCP 功能，AI 可以根据需要自动判断是否使用以下工具：

- **视觉理解 MCP**: 启用后，AI 可以识别和理解图片内容
- **联网搜索 MCP**: 启用后，AI 可以搜索最新信息。当用户明确要求"搜索"、"查找"、"上网"等时，或提出需要实时信息的问题时自动使用
- **网页阅读 MCP**: 启用后，AI 可以读取网页内容。当输入中包含 URL 或"访问"、"读取网页"等关键词时自动使用
- **开源仓库 MCP**: 启用后，AI 可以读取 GitHub 等代码仓库。当输入中包含仓库链接或"项目"、"代码"、"PR"、"Issue"等关键词时自动使用

**使用说明**：
- MCP 功能完全由 AI 智能判断是否需要使用
- 用户也可以在 Prompt 中明确要求使用特定功能（如"帮我搜索一下..."）
- 只有勾选的 MCP 才会被激活，未勾选的不会使用
...
- **智谱 AI**: glm-4, glm-4-flash（支持 MCP 功能）
...
A: MCP（Model Context Protocol）是智谱 AI 提供的增强功能，可以让 AI 调用外部工具：
- **视觉理解**: AI 可以"看"图片并理解内容
- **联网搜索**: AI 可以获取最新信息
- **网页阅读**: AI 可以读取指定网页
- **开源仓库**: AI 可以读取 GitHub 等代码仓库

AI 会根据你的输入智能判断是否需要使用这些工具，你也可以明确要求使用。

### Q: 如何停止正在进行的 AI 处理？

A: 直接按 `Escape` 键即可停止。停止后不会插入任何内容到笔记中。
