...
#### 1. 修复智谱 AI API 对接问题
- 修复了因 `max_tokens` 参数值过大导致的 API 400 错误
- 将默认 `maxTokens` 从 2000 降低到 1000，避免超出平台限制
- 移除自动端点转换逻辑，直接使用用户配置的端点
- 优化参数处理，只在明确指定时才添加可选参数
- 更新智谱 AI 模型列表，使用官方标准模型 ID（glm-4、glm-4-plus、glm-3-turbo）
- 更新设置界面说明，明确表示通用端点和 Coding 端点均可使用

### 技术细节

#### 修改文件
- `providers/zhipuProvider.ts` - 移除自动端点转换，优化参数处理
- `main.ts` - 调整默认配置和模型列表
